...
import static java.time.temporal.TemporalAdjusters.*;  //Factory, all static, class. nextOrSame
...
class DerivedDateTimeTest {
    ...
    public static void testEarlyRetirement() {
        LocalDate bday = LocalDate.of(1996,11,9);
...
//        LocalDate election = nov2.with(TemporalAdjusters.next(DayOfWeek.TUESDAY));
//        System.out.println(election);

        LocalDate nov1 = LocalDate.of(2024, 11, 1);
        LocalDate election = nov1.with(nextOrSame(MONDAY)).plusDays(1); //same as `laborDay = code above`
        System.out.println(election);
    }
    ...
    public static void testAnniversary() {
        ...
        LocalDate anny = wedding.plusYears(50);
        LocalDate party = anny.with(nextOrSame(SATURDAY));
